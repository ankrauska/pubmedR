...
pmApi2df <- function(P, format="bibliometrix"){
  ...
  df <- data.frame(AU=rep(NA,n), AF="NA",TI="NA", SO="NA", SO_CO=NA, LA=NA, DT=NA,DE=NA,ID=NA,MESH=NA,AB="NA",BACKGROUND = "NA", METHODS = "NA", RESULTS="NA", CONCLUSIONS = "NA", C1=NA,CR="NA",
                   TC=NA, SN=NA, J9=NA, JI=NA, PY=NA, PY_IS=NA, VL=NA, DI=NA, PG=NA, GRANT_ID=NA, GRANT_ORG=NA, UT=NA, PMID=NA,
                   DB="PUBMED", AU_UN=NA, CT= NA, stringsAsFactors = FALSE)
#TODO: Added CT for citations
  ...
  for (i in 1:n) {
    ...
    if(length(ind) == 4){
      df$BACKGROUND[i] = a[ind[1]]
      df$METHODS[i] = a[ind[2]]
      df$RESULTS[i] = a[ind[3]]
      df$CONCLUSIONS[i] = a[ind[4]]
    }
...
#TODO: Added citations
    ##Citations
    CT_ID <- which(items %in% "PubmedData.ReferenceList.Reference.ArticleIdList.ArticleId.text")
    df$CT_ID[i] <- paste(a[CT_ID], collapse=";")
  }
